<template>
  <v-container fluid>
    <v-tabs v-model="tab" background-color="secondary" dark>
      ...
      <v-spacer></v-spacer>
      <v-toolbar-items>
        <v-btn v-if="uid === '6f2eac1b-5b1d-49ce-a4b7-f9089128f836'" color="warning"
        ...
        <v-row>
          <v-col cols="12">
            <v-card class="mx-auto">
              <v-carousel v-model="selected" delimiter-icon="mdi-account" next-icon="mdi-account-arrow-right"
                prev-icon="mdi-account-arrow-left" :show-arrows-on-hover="true">
                <v-carousel-item v-for="(patient, i) in match_items" :key="`${i}-${patient.id}`">
                  <v-card class="mx-auto" height="100%">
                    ...
                    <v-list dense light style="max-height: 400px; overflow-y: auto;">
                      ...
                      <v-list-item v-for="(name, j) in patient.name" :key="`${j}-${name.use}`">
                        <v-list-item-content>{{ $t("surname") }} ({{
                          name.use
                          }})</v-list-item-content>
                      ...
                      <v-list-item>
                        <v-list-item-content>Sub County:</v-list-item-content>
                        <v-list-item-content class="align-end">
                          {{ patient.subcounty }}
                        </v-list-item-content>
                      </v-list-item>
                      ...
                      <v-list-item>
                        <v-list-item-content>Country of residence:</v-list-item-content>
                        <v-list-item-content class="align-end">
                          {{ patient.district}}
                        </v-list-item-content>
                      </v-list-item>
                      ...
                      <v-list-item>
                        <v-list-item-content>Next of Kin :</v-list-item-content>
                        <v-list-item-content class="align-end">
                          "Not Set"
                        </v-list-item-content>
                      </v-list-item>

                      <v-list-item v-for="(telecom, k) in patient.telecom" :key="`${k}-${telecom.system}`">
                      ...
                      <v-list-item v-for="(id, l) in patient.identifier" :key="`${l}-${id.system}`">
          ...
          <v-col cols="12">
            <v-card class="mx-auto">
              ...
              <v-data-table v-model="breaks" :headers="match_headers" :items="match_items" :items-per-page="20"
                :footer-props="{
      'items-per-page-text': this.$t('row_per_page'),
    }" :no-data-text="$t('no_data')" class="elevation-1 text-capitalize" item-key="fid" show-select />
              <v-card-actions>
                <v-spacer />
                <v-btn class="warning" :disabled="breaks.length === 0 || match_items.length < 2" @click="breakMatch()">
                  {{ $t("break_matches") }}
                </v-btn>
              </v-card-actions>
            </v-card>
            <v-card class="mx-auto">
              ...
              <v-data-table v-model="unbreaks" :headers="match_headers" :items="break_items" :items-per-page="20"
                :footer-props="{
                  'items-per-page-text': this.$t('row_per_page'),
                }" class="elevation-1 text-capitalize" :no-data-text="$t('no_data')" item-key="id" show-select />
              <v-card-actions>
                <v-spacer />
                <v-btn class="accent" :disabled="unbreaks.length === 0" @click="revertBreak()">
                  {{ $t("revert_break") }}
                </v-btn>
              </v-card-actions>
            </v-card>
              ...
              <v-expansion-panels popout>
                <v-expansion-panel v-for="(event, i) in matchEvents" :key="i">
                  ...
                  <v-expansion-panel-content>
                    ...
                    <v-row v-for="(detail, j) in event.matchData" v-else :key="j">
                      <v-col cols="6">
                        <v-card elevation="12" hover>
                          ...
                          <v-card-text>
                            <v-data-table :headers="matchRuleHeaders" :items="detail.decisionRule" :items-per-page="20"
                              item-key="id">
                              <template v-slot:item.details="{ item }">
                                ...
                                <template v-if="item.details.threshold">
                                  Threshold
                                  <v-chip color="red" dark>
                                    {{ item.details.threshold }} </v-chip><br />
                                </template>
                      ...
                      <v-col cols="6">
                        <v-switch v-model="advancedView" label="View Advanced Details" />
                        <template v-if="advancedView">
                          <v-card>
                            <v-card-text>
                              <v-textarea filled color="deep-purple" label="Elasticsearch Query" rows="10"
                                :value="detail.query" />
                            </v-card-text>
                          </v-card>
                          <v-card>
                            <v-card-text>
                              <v-textarea filled color="deep-purple" label="Elasticsearch Automatches Results" rows="10"
                                :value="detail.autoMatches" />
                            </v-card-text>
                          </v-card>
                          <v-card>
                            <v-card-text>
                              <v-textarea filled color="deep-purple" label="Elasticsearch Potential Matches Results"
                                rows="10" :value="detail.potentialMatches" />
                            </v-card-text>
                          </v-card>
                          <v-card>
                            <v-card-text>
                              <v-textarea filled color="deep-purple" label="Elasticsearch Conflicts Matches Results"
                                rows="10" :value="detail.conflictsMatchResults" />
                            </v-card-text>
                          </v-card>
                        </template>
                      </v-col>
                ...
                for (let entry of resp.data.entry) {
                  ...
                  if (
                    ...
                    this.break_items.unshift({
                      ...
                      phone: phone,
                      nationality: patient.address[0].country,
                      maritalStatus: "Not Set",
                      facility: patient.managingOrganization.display,
                    });
                  } else {
                ...
                if (
                  ...
                  if (
                    ...
                    this.match_items.unshift({
                      ...
                      maritalStatus: "Not Set",
                      facility: patient.managingOrganization.display,
                      village: patient.address.find(addr => addr.extension.some(ext => ext.url === 'http://fhir.openmrs.org/ext/address'))
                        .extension.find(ext => ext.url === 'http://fhir.openmrs.org/ext/address')
                        .extension.find(subExt => subExt.url === 'http://fhir.openmrs.org/ext/address#village')
                        .valueString,
                      parish: patient.address.find(addr => addr.extension.some(ext => ext.url === 'http://fhir.openmrs.org/ext/address'))
                        .extension.find(ext => ext.url === 'http://fhir.openmrs.org/ext/address')
                        .extension.find(subExt => subExt.url === 'http://fhir.openmrs.org/ext/address#parish')
                        .valueString,
                      subcounty: patient.address.find(addr => addr.extension.some(ext => ext.url === 'http://fhir.openmrs.org/ext/address'))
                        .extension.find(ext => ext.url === 'http://fhir.openmrs.org/ext/address')
                        .extension.find(subExt => subExt.url === 'http://fhir.openmrs.org/ext/address#subcounty')
                        .valueString,
                      county: patient.address.find(addr => addr.extension.some(ext => ext.url === 'http://fhir.openmrs.org/ext/address'))
                        .extension.find(ext => ext.url === 'http://fhir.openmrs.org/ext/address')
                        .extension.find(subExt => subExt.url === 'http://fhir.openmrs.org/ext/address#subcounty')
                        .valueString,
                      district: patient.address[0].district,
                      countryOfResidence: patient.address[0].district
