    ...
    <v-row>
      ...
      <v-col cols="6">
        <v-card class="mx-auto">
            ...
            >
              <v-card class="mx-auto" height="100%">
                <v-toolbar color="secondary" dark>
                  <v-toolbar-title class="font-weight-bold">CRUID: {{ uid }}</v-toolbar-title>
                  <v-spacer></v-spacer>
                  {{ selected+1 }} / {{ match_count }}
                </v-toolbar>
                <v-list dense light height="100%">
                  <v-list-item>
                    <v-list-item-content>Submitting System:</v-list-item-content>
                    <v-list-item-content class="align-end">{{patient.system}}</v-list-item-content>
                  </v-list-item>
                  <v-list-item
                  ...
                  >
                    ...
                    <v-list-item-content class="align-end text-capitalize">{{name.given.join(" ")}} {{name.family}}</v-list-item-content>
                  </v-list-item>
                  <v-list-item>
                    <v-list-item-content>Gender:</v-list-item-content>
                    <v-list-item-content class="align-end">{{patient.gender}}</v-list-item-content>
                  </v-list-item>
                  <v-list-item>
                    <v-list-item-content>Birth Date:</v-list-item-content>
                    <v-list-item-content class="align-end">{{patient.birthdate}}</v-list-item-content>
                  </v-list-item>
                  <v-list-item
                    v-for="(telecom, k) in patient.telecom"
                    :key="`${k}-${telecom.system}`"
                  >
                    <v-list-item-content class="text-capitalize">{{telecom.system}}:</v-list-item-content>
                    <v-list-item-content class="align-end">{{telecom.value}}</v-list-item-content>
                  </v-list-item>
                  <v-list-item
                  ...
                  >
                    ...
                    <v-list-item-content class="align-end">{{id.value}}</v-list-item-content>
                  </v-list-item>
                </v-list>
              </v-card>
            </v-carousel-item>
          ...
          <v-list two-lines class="info" dark>
            <v-list-item>
...
//        { id: "rec-978-org-", gender: "f", birthdate: "1914-09-20", given: "kabyanga", family: "anicia", nin: "CF59866315NPOE", art: "FPL-944851", phone: "772 702269" }
  ...
  mounted() {
    this.systems[ process.env.VUE_APP_SYSTEM_OPENMRS ] = process.env.VUE_APP_SYSTEM_NAME_OPENMRS
    this.systems[ process.env.VUE_APP_SYSTEM_ART ] = process.env.VUE_APP_SYSTEM_NAME_ART
    this.systems[ process.env.VUE_APP_SYSTEM_NIN ] = process.env.VUE_APP_SYSTEM_NAME_NIN
    this.$http.get( process.env.VUE_APP_FHIR + "/Patient?_elements=link&_id="+this.$route.params.clientId ).then( (response) => {
      let uid = response.data.entry[0].resource.link[0].other.reference.substring(8)
      this.$http.get( process.env.VUE_APP_FHIR + "/Patient?_include=Patient:link&_id="+uid ).then( ( resp ) => {
        for ( let entry of resp.data.entry ) {
          let patient = entry.resource
          if ( patient.meta.tag && patient.meta.tag.find( tag => tag.code === process.env.VUE_APP_CRUID_TAG ) !== undefined ) {
            this.uid = patient.id
          } else {
            if ( patient.id === this.$route.params.clientId ) this.selected = this.match_count
            ...
            try {
              phone = patient.telecom.find( phone => phone.system = "phone" ).value
            } catch( err ) {
              phone = ""
            }
            this.match_items.push( {
              fid: patient.id, selectIdx: this.match_count, system: systemName,
              id: recordId, gender: patient.gender, birthdate: patient.birthDate,
              name: patient.name, telecom: patient.telecom, identifier: patient.identifier,
              family: name.family, given: name.given.join(" "),
              phone: phone, art: art, nin: nin
            } )
            this.match_count++
          }
        }
