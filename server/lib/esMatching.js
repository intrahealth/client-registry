...
const buildQuery = (sourceResource, decisionRule) => {
  ...
  for (const ruleField in decisionRule.fields) {
    ...
    if (
      ...
      !(pathValue.length === 1 && pathValue[0] === undefined)
    ) {
    ...
    if (
      rule.algorithm === "phonetic" &&
      decisionRule.matchingType === "probabilistic"
    ) {
      logger.error(
        "Phonetic is not supported for probabilistic matching, use it for deterministic matching only. Aborting matching process"
      );
      return {};
    }
    ...
    if (
      !isNaN(parseFloat(rule.weight)) &&
      decisionRule.matchingType === "deterministic"
    ) {
  ...
  if (
    (!decisionRule.filters || Object.keys(decisionRule.filters).length === 0) &&
    Object.keys(esquery.query.function_score.query.bool).length === 0
  ) {
      ...
      axios
        ...
        .then((response) => {
          ...
          if (
            ...
            response.data.hits.total.value == documents.length ||
            !response.data._scroll_id
          ) {
            scroll_id = null;
          } else {
        ...
        .catch((err) => {
          if (err.response && err.response.status === 429) {
            logger.warn(
              "ES is overloaded with too many requests, delaying for 2 seconds"
            );
            setTimeout(() => {
              return callback(null);
            }, 2000);
...
const performMatch = (
  { sourceResource, currentGoldenLink, ignoreList },
  callback
) => {
  logger.info(
    "Finding matches for patient " +
      JSON.stringify(sourceResource.identifier, 0, 2)
  );
  let error = false;
  let maxScore;
    ...
    async.eachSeries(
      ...
      (decisionRule, nxtRule) => {
        if (
          ...
          logger.error(
            "Matching type is not specified under decision rule, should be either deterministic or probabilistic"
          );
        ...
        getESDocument(esquery, (err, documents) => {
          ...
          for (const hit of documents) {
            ...
            if (
              score > parseFloat(maxScore) ||
              (!maxScore && score >= decisionRule.autoMatchThreshold)
            ) {
              resourceID = id;
              maxScore = score;
            }
              ...
              if (resourceID) {
                ...
                fhirWrapper.getResource(
                  ...
                  (resourceData) => {
                    ...
                    fhirMatchResults.entry = resourceData.entry.filter(
                      (entry) => {
                          ...
                          for (let link of entry.resource.link) {
                            let sameScore = ESMatches.find((esmatch) => {
                              return esmatch.autoMatchResults.find((autoM) => {
                                return (
                                  autoM["_score"] === maxScore &&
                                  entry.resource.id === autoM["_id"] &&
                                  autoM["_id"] !== resourceID
                                );
                              });
                            });
                            if (
                              link.other.reference.split("/")[1] ===
                                currentGoldenLink &&
                              sameScore
                            ) {
            ...
            if (resourceID) {
              ...
              for (let fhirMatch of fhirMatchResults.entry) {
                let sameGoldenID = fhirMatch.resource.link.find((link) => {
                  return (
                    link && link.other && link.other.reference === goldenID
                  );
                });
                if (sameGoldenID) {
              ...
              for (const index in ESMatches) {
                ...
                for (let autoIndex = 0; autoIndex < total; autoIndex++) {
                  let autoMatch =
                    ESMatches[index].autoMatchResults[
                      autoIndex - deletedAutoIndex
                    ];
                  ...
                  if (!exist) {
                    ...
                    ESMatches[index].autoMatchResults.splice(
                      autoIndex - deletedAutoIndex,
                      1
                    );
                    deletedAutoIndex++;
              ...
              for (
                ...
                potentialIndex < total;
                potentialIndex++
              ) {
                let potentialMatch =
                  FHIRPotentialMatches.entry[potentialIndex - deletedIndex]
                    .resource;
                ...
                if (sameGoldenID) {
                  FHIRPotentialMatches.entry.splice(
                    potentialIndex - deletedIndex,
                    1
                  );
                  deletedIndex++;
                  ...
                  for (let esIndex in ESMatches) {
                    ...
                    for (let esPotIndex in esmatch.potentialMatchResults) {
                      let esPotMatch =
                        esmatch.potentialMatchResults[esPotIndex];
                      if (esPotMatch["_id"] === potentialMatch.id) {
                        ESMatches[esIndex].potentialMatchResults.splice(
                          esPotIndex,
                          1
                        );
