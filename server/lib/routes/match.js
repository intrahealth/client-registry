...
const matchIssuesURI = URI(config.get("systems:CRBaseURI"))
  .segment("matchIssues")
  .toString();
const matchAutoURI = URI("http://openclientregistry.org/fhir")
  .segment("automatch")
  .toString();
const humanAdjudicationURI = URI(config.get("systems:CRBaseURI"))
        ...
        for (let patient of resolves) {
          if (patient.uid !== patient.ouid) {
            ...
            for (let index in patientResource.resource.link) {
              let linkID =
                patientResource.resource.link[index].other.reference.split(
                  "/"
                )[1];
            ...
            for (let index in oldCRUID.resource.link) {
              let patientID =
                oldCRUID.resource.link[index].other.reference.split("/")[1];
            ...
            async.each(
              ...
              (resolvePatient, nxtRes) => {
                ...
                if (resolvePatient.id !== resolvingFrom) {
                  ...
                  let joinedThisCRUID = resolves.find((resolve) => {
                    return (
                      resolve.uid !== resolve.ouid &&
                      resolve.uid === resolvePatient.uid
                    );
                  });
                  if (
                    resolvePatient.uid === resolvePatient.ouid &&
                    !joinedThisCRUID
                  ) {
                    return nxtRes();
                  }
                ...
                let resolvePatientResource = modifiedResourceData.entry.find(
                  (entry) => {
                    return entry.resource.id === resolvePatient.id;
                  }
                );
                if (!resolvePatientResource) {
                  resolvePatientResource = originalResourceData.entry.find(
                    (entry) => {
                      return entry.resource.id === resolvePatient.id;
                    }
                  );
                ...
                matchingTool.performMatch(
                  ...
                  ({
                    ...
                    FHIRConflictsMatches.entry =
                      FHIRConflictsMatches.entry.filter((entry) => {
                    ...
                    FHIRConflictsMatches.entry = FHIRAutoMatched.entry.filter(
                      (entry) => {
                        let needsResolving = true;
                        let link;
                        if (entry.resource.link) {
                          link =
                            entry.resource.link[0].other.reference.split(
                              "/"
                            )[1];
                        }
                        if (resolvePatient.uid === link) {
                    ...
                    FHIRPotentialMatches.entry =
                      FHIRPotentialMatches.entry.filter((entry) => {
                        let needsResolving = true;
                        let link;
                        if (entry.resource.link) {
                          link =
                    ...
                    async.parallel(
                      {
                        autoMatches: async (callback) => {
                          if (flagType === "autoMatches" && removeFlag) {
                            if (
                              ...
                              for (let tagIndex in resolvePatientResource
                                .resource.meta.tag) {
                                let tag =
                                  resolvePatientResource.resource.meta.tag[
                                    tagIndex
                                ...
                                if (
                                  tag.system === matchAutoURI &&
                                  tag.code === "autoMatches"
                                ) {
                                  resolvePatientResource.resource.meta.tag.splice(
                                    tagIndex,
                                    1
                                  );
                              ...
                              await fhirWrapper["$meta-delete"]({
                                ...
                                logger.error(
                                  "An error has occured while removing auto match tag"
                                );
                                errorOccured = true;
                            ...
                            let tagExist =
                              resolvePatientResource.resource.meta.tag.find(
                                (tag) => {
                                  return (
                                    tag.system === matchAutoURI &&
                                    tag.code === "autoMatches"
                        ...
                        potentialMatches: async (callback) => {
                          if (
                            FHIRPotentialMatches.entry.length === 0 ||
                            (flagType === "potentialMatches" && removeFlag)
                          ) {
                            if (
                              resolvePatientResource.resource.meta &&
                              resolvePatientResource.resource.meta.tag
                              ...
                              await fhirWrapper["$meta-delete"]({
                                ...
                                logger.error(
                                  "An error has occured while removing potential match tag"
                                );
                                errorOccured = true;
                                return callback(null);
                              });
                        ...
                        conflictsMatches: async (callback) => {
                          if (
                            FHIRConflictsMatches.entry.length === 0 ||
                            (flagType === "conflictMatches" && removeFlag)
                          ) {
                            if (
                              resolvePatientResource.resource.meta &&
                              resolvePatientResource.resource.meta.tag
                              ...
                              await fhirWrapper["$meta-delete"]({
                                ...
                                logger.error(
                                  "An error has occured while removing conflict match tag"
                                );
                                errorOccured = true;
                                return callback(null);
                              });
                      ...
                      () => {
                        ...
                        modifiedResourceData.entry.push({
                          ...
                          request: {
                            method: "PUT",
                            url:
                              "Patient/" + resolvePatientResource.resource.id,
                          },
                        });
                        fhirWrapper.saveResource(
                          { resourceData: modifiedResourceData },
...
router.post("/break-match", (req, res) => {
  ...
  if (!Array.isArray(ids)) {
    logger.error(
      "Invalid requ;est, expected an array of IDs but non was found in " + ids
    );
    ...
    fhirWrapper.getResource(
      ...
      (resourceData) => {
        ...
        for (const entry of resourceData.entry) {
          ...
          operSummary.editingResource =
            entry.resource.resourceType + "/" + entry.resource.id;
          if (
            !entry.resource.link ||
            (entry.resource.link && entry.resource.link.length === 0)
          ) {
          ...
          for (const link of entry.resource.link) {
            ...
            for (const bundleIndex in bundle.entry) {
              if (
                bundle.entry[bundleIndex].resource.id ===
                goldenRec2RecoLink[link.other.reference].newGoldenRecordId
              ) {
            ...
            goldenRec2RecoLink[link.other.reference].recordId.push(
              entry.resource.id
            );
          ...
          (goldenRecords) => {
            ...
            for (const goldenRecord of goldenRecords.entry) {
              for (const linkIndex in goldenRecord.resource.link) {
                if (
                  !ids.includes(
                    goldenRecord.resource.link[linkIndex].other.reference
                  )
                ) {
                  for (const index in operationSummary) {
                    operationSummary[index].brokenResources.push(
                      goldenRecord.resource.link[linkIndex].other.reference
                    );
                  }
                }
            ...
            for (const index in resourceData.entry) {
              resourceData.entry[index].request = {
                method: "PUT",
                url:
                  resourceData.entry[index].resource.resourceType +
                  "/" +
                  resourceData.entry[index].resource.id,
              };
              ...
              for (const goldenRecord of goldenRecords.entry) {
                for (const linkIndex in goldenRecord.resource.link) {
                  if (
                    goldenRecord.resource.link[linkIndex].other.reference ===
                    "Patient/" + resourceData.entry[index].resource.id
                  ) {
              ...
              for (const linkToGold of resourceData.entry[index].resource
                .link) {
                ...
                if (!goldenRecord) {
                  for (const index in operationSummary) {
                    if (
                      operationSummary[index].editingResource ===
                      resourceData.entry[index].resource.resourceType +
                        "/" +
                        resourceData.entry[index].resource.id
                    ) {
                      operationSummary[index].outcome = "8";
                      operationSummary[index].outcomeDesc =
                        "Golden record was not found on the database";
                    }
                  }
                ...
                for (const linkedRec of goldenRecord.resource.link) {
                  const shareSameGolden = goldenRec2RecoLink[
                    "Patient/" + goldenRecord.resource.id
                  ].recordId.find((mappedLink) => {
                    return (
                      "Patient/" + mappedLink === linkedRec.other.reference
                    );
                  });
                  if (!shareSameGolden) {
                    ...
                    const extExist = resourceData.entry[
                      index
                    ].resource.extension.find((extension) => {
                      return (
                        extension.url ===
                          config.get("systems:brokenMatch:uri") &&
                        extension.valueReference.reference ===
                          linkedRec.other.reference
                      );
                    });
                    if (!extExist) {
              ...
              if (resourceData.entry[index].resource.link.length > 1) {
                ...
                for (const linkToGold of resourceData.entry[index].resource
                  .link) {
                  let newGoldenRecord =
                    goldenRec2RecoLink[linkToGold.other.reference]
                      .newGoldenRecordId;
                  newGoldenRecord = "Patient/" + newGoldenRecord;
                  if (!goldenRecordToUse) {
                    goldenRecordToUse = newGoldenRecord;
                    totalLinks =
                      goldenRec2RecoLink[linkToGold.other.reference].recordId
                        .length;
                  } else {
                    if (
                      goldenRec2RecoLink[linkToGold.other.reference].recordId
                ...
                const linkToGoldId =
                  resourceData.entry[index].resource.link[0].other.reference;
                const newGoldenRecord =
                  goldenRec2RecoLink[linkToGoldId].newGoldenRecordId;
                resourceData.entry[index].resource.link = [
                  {
                    other: {
            ...
            for (const entryIndex in bundle.entry) {
              ...
              const fromDB =
                ...
                entry.resource.meta.tag.find((tag) => {
                  return (
                    tag.code === config.get("codes:goldenRecord") &&
                    entry.resource.fromDB
                  );
                });
              if (fromDB) {
              ...
              for (const resEntry of resourceData.entry) {
                const found = resEntry.resource.link.find((link) => {
                  return (
                    link.other.reference === "Patient/" + entry.resource.id
                  );
                });
                exist = found;
              }
            ...
            if (linkedRecordsQuery) {
                ...
                (linkedRecordsData) => {
                  for (const linkedRecord of linkedRecordsData.entry) {
                    ...
                    for (const index in resourceData.entry) {
                      const youBrokeMe =
                        resourceData.entry[index].resource.extension &&
                        resourceData.entry[index].resource.extension.find(
                          (extension) => {
                            return (
                              extension.url ===
                                config.get("systems:brokenMatch:uri") &&
                              extension.valueReference.reference ===
                                "Patient/" + linkedRecord.resource.id
                            );
                          }
                        );
                      if (youBrokeMe) {
                        ...
                        const extExist = linkedRecord.resource.extension.find(
                          (extension) => {
                            return (
                              extension.url ===
                                config.get("systems:brokenMatch:uri") &&
                        ...
                        if (!extExist) {
                          ...
                          bundle.entry.push({
                            ...
                            request: {
                              method: "PUT",
                              url:
                                linkedRecord.resource.resourceType +
                                "/" +
                                linkedRecord.resource.id,
                            },
                          });
                        }
                      }
                  ...
                  if (noLink.length > 0 || notProcessed.length > 0) {
                    ...
                    if (notProcessed.length > 0) {
                      respObj.issue.push({
                        ...
                        code: "processing",
                        diagnostics:
                          "Invalid ID format submitted for " +
                          notProcessed.join(", ") +
                          " but no link found",
                      });
                    }
              ...
              for (const index in operationSummary) {
                operationSummary[index].outcome = "8";
                operationSummary[index].outcomeDesc =
                  "Links to records were not found";
              }
              saveAuditEvent(() => {
                return res.status(500).json({
                  resourceType: "OperationOutcome",
...
router.get("/potential-matches/:id", (req, res) => {
  ...
  fhirWrapper.getResource(
    ...
    (patient) => {
       ...
       */
      generateScoreMatrix(
        { patient: JSON.parse(JSON.stringify(patient)), level: "childMatches" },
        () => {
  ...
  function generateScoreMatrix({ patient, level }, callback) {
    ...
    matchingTool.performMatch(
      {
        sourceResource: JSON.parse(JSON.stringify(patient)),
      ...
      ({
        ...
        if (patient.extension) {
          for (let id of patient.extension) {
            ...
            primaryPatient[propertyName] = id.valueString
              ? id.valueString
              : id.valueDate;
          }
        }
        ...
        if (
          level != "childMatches" &&
          !config.get("matching:resolvePotentialOfPotentials")
        ) {
          return callback();
        }
        async.series(
          {
            auto: (callback) => {
              async.eachSeries(
                ...
                (autoMatched, nxtAutoMatched) => {
                  const validSystem = generalMixin.getClientIdentifier(
                    autoMatched.resource
                  );
                  ...
                  generateScoreMatrix(
                    {
                      patient: autoMatched.resource,
                      level: "grandChildMatches",
                    },
                    () => {
                      return nxtAutoMatched();
                    }
            ...
            potential: (callback) => {
              async.eachSeries(
                FHIRPotentialMatches.entry,
                (potentialMatch, nxtPotMatch) => {
                  const validSystem = generalMixin.getClientIdentifier(
                    potentialMatch.resource
                  );
                  if (matrixExist(validSystem.value)) {
                    return nxtPotMatch();
                  }
                  generateScoreMatrix(
                    {
                      patient: potentialMatch.resource,
                      level: "grandChildMatches",
                    },
                    () => {
                      return nxtPotMatch();
            ...
            conflicts: (callback) => {
              async.eachSeries(
                FHIRConflictsMatches.entry,
                (conflictMatch, nxtConflictMatch) => {
                  const validSystem = generalMixin.getClientIdentifier(
                    conflictMatch.resource
                  );
                  if (matrixExist(validSystem.value)) {
                  ...
                  generateScoreMatrix(
                    {
                      patient: conflictMatch.resource,
                      level: "grandChildMatches",
                    },
                    () => {
                      return nxtConflictMatch();
...
router.get(`/get-match-issues`, (req, res) => {
  const clientIDURI = URI(config.get("systems:CRBaseURI"))
    .segment("clientid")
    .toString();
  fhirWrapper.getResource(
    {
      ...
      for (let entry of issues.entry) {
        ...
        const validSystem =
          ...
          entry.resource.identifier.find((identifier) => {
            return (
              "http://openclientregistry.org/fhir/sourceid" && identifier.value
            );
          });
    ...
    fhirWrapper.getResource(
      ...
      (resourceData) => {
        ...
        for (const idPair of ids) {
          ...
          const clientIdTag1 =
            resource1.resource.meta &&
            resource1.resource.meta.tag &&
            resource1.resource.meta.tag.find((tag) => {
              return (
                tag.system ===
          ...
          const clientIdTag2 =
            resource2.resource.meta &&
            resource2.resource.meta.tag &&
            resource2.resource.meta.tag.find((tag) => {
              return (
                tag.system ===
                URI(config.get("systems:CRBaseURI"))
                  .segment("clientid")
          ...
          if (resource1 && resource1.resource.extension) {
            for (const index in resource1.resource.extension) {
              ...
              if (
                ext.url === config.get("systems:brokenMatch:uri") &&
                ext.valueReference &&
                ext.valueReference.reference === id2
              ) {
                ...
                bundle.entry.push({
                  ...
                  request: {
                    method: "PUT",
                    url:
                      resource1.resource.resourceType +
                      "/" +
                      resource1.resource.id,
                  },
                });
              }
            }
          ...
          if (resource2 && resource2.resource.extension) {
            for (const index in resource2.resource.extension) {
              ...
              if (
                ext.url === config.get("systems:brokenMatch:uri") &&
                ext.valueReference &&
                ext.valueReference.reference === id1
              ) {
                ...
                bundle.entry.push({
                  ...
                  request: {
                    method: "PUT",
                    url:
                      resource2.resource.resourceType +
                      "/" +
          ...
          if (!auditedId.includes(id1)) {
            ...
            operSummary.editingResource =
              resource1.resource.resourceType + "/" + resource1.resource.id;
            operSummary.CRUID = resource2.resource.link[0].other.reference;
            if (dontSaveChanges) {
              operSummary.outcome = "8";
            }
            operSummary.unBreakResources.push(
              resource2.resource.resourceType + "/" + resource2.resource.id
            );
            operationSummary.push(operSummary);
          } else {
            for (const index in operationSummary) {
              ...
              if (
                oper.editingResource ===
                resource1.resource.resourceType + "/" + resource1.resource.id
              ) {
                operationSummary[index].unBreakResources.push(
                  resource2.resource.resourceType + "/" + resource2.resource.id
                );
              }
            }
        ...
        if (!dontSaveChanges) {
          fhirWrapper.saveResource(
            ...
            (err) => {
              if (err) {
                logger.error(
                  "An error has occured while saving unbroken matches"
                );
                return res.status(500).json({
                  resourceType: "OperationOutcome",
                  issue: [
              ...
              async.eachSeries(
                ...
                (entry, nxtEntry) => {
                  ...
                  if (clientID) {
                    ...
                    matchMixin.addPatient(
                      clientID,
                      patientsBundle,
                      (err, response, operSum) => {
                        // operationSummary.push(operSum);
                        const tmpAuditBundle =
                          matchMixin.createAddPatientAudEvent(
                            operationSummary,
                            req
                          );
                        auditBundle.entry = auditBundle.entry.concat(
                          tmpAuditBundle.entry
                        );
