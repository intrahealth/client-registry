  ...
  form.parse(req, (err, fields, files) => {
    ...
    request.get(options, (err, response, body) => {
      ...
      const bundle = {
        ...
        extension: [
          {
            url: URI(config.get("structureDefinition:uri"))
              .segment("StructureDefinition")
              .segment("OCRUserDetails")
              .toString(),
            extension: [
              {
                url: "username",
  ...
  form.parse(req, (err, fields, files) => {
    ...
    request.get(options, (err, response, body) => {
      ...
      let userExt = body.extension.find((ext) => {
        return (
          ext.url ===
          "http://openclientregistry.org/fhir/StructureDefinition/OCRUserDetails"
        );
      });
  ...
  form.parse(req, (err, fields, files) => {
    ...
    request.get(options, (err, response, body) => {
      if (!isJSON(body)) {
        logger.error(body);
        logger.error(
          "Non JSON has been returned while getting user information for user " +
            req.query.username
        );
        return res.status(500).json({
        ...
        for (var i in extensions) {
          if (extensions[i].url.includes("OCRUserDetails")) {
            ...
            if (hash === password) {
              const salt = crypto.randomBytes(16).toString("hex");
              const password = crypto
                .pbkdf2Sync(fields.newpassword, salt, 1000, 64, "sha512")
                .toString("hex");
    ...
    if (numMatches == 0) {
      ...
      for (var i in extensions) {
        if (extensions[i].url.includes("OCRUserDetails")) {
          ...
          const hash = crypto
            .pbkdf2Sync(req.query.password, salt, 1000, 64, "sha512")
            .toString("hex");
          ...
          if (hash === password) {
            ...
            logger.info(
              `Successfully Authenticated user ${req.query.username}`
            );
            return res.status(200).json({
              token,
              userID: user.id,
