...
router.get("/ValueSet/:id/\\$expand", (req, res) => {
  fhirAxios
    .expand(req.params.id, req.query)
    .then((resource) => {
...
function pixmRequest({ req }, callback) {
  ...
  if (Object.keys(otherQueries).length > 0) {
    ...
    outcome.issue.push({
      ...
      diagnostics:
        ...
        unknownQueries.join("&") +
        "'. Value search parameters for this search are: [sourceIdentifier, targetSystem]",
    });
  }
  if (!sourceIdentifier) {
    outcome.issue.push({
    ...
    if (srcId.length !== 2) {
      outcome.issue.push({
        ...
        code: "processing",
        diagnostics:
          "Invalid value for parameter 'sourceIdentifier', the value must include both the Patient Identity Domain (i.e., Assigning Authority) and the identifier value, separated by a '|' i.e nationalid|123.",
      });
    }
  }
  ...
  async.parallel(
    {
      ...
      patients: (callback) => {
        ...
        if (
          req.connection &&
          typeof req.connection.getPeerCertificate === "function"
        ) {
        ...
        matchMixin.addPatient(
          clientID,
          patientsBundle,
          (err, responseBundle, responseHeaders, operationSummary) => {
            if (err) {
              return callback(null, {
                code: 500,
                err,
    ...
    (err, results) => {
      ...
      for (let entry of results.patients.responseBundle.entry) {
        let exists = filteredResponseBundle.findIndex((fil) => {
          return fil.response.location.startsWith(
            entry.response.location.split("/_history")[0]
          );
        });
        if (exists === -1) {
          ...
          let replaceIndex = filteredResponseBundle.findIndex((fil) => {
            return (
              parseInt(fil.response.etag) < parseInt(entry.response.etag) &&
              fil.response.location.startsWith(
                entry.response.location.split("/_history")[0]
              )
            );
          });
      ...
      res.setHeader(
        "Location",
        JSON.stringify(results.patients.responseHeaders.patientID)
      );
      res.setHeader(
        "LocationCRUID",
        JSON.stringify(results.patients.responseHeaders.CRUID)
      );
      res.status(code).json(filteredResponseBundle);

      const auditBundle = matchMixin.createAddPatientAudEvent(
        results.patients.operationSummary,
        req
      );
...
function saveResource(req, res) {
  ...
  if (resourceType !== "Patient") {
    ...
    if (
      !resource.resourceType ||
      !resource.identifier ||
      (resource.identifier && resource.identifier.length === 0)
    ) {
      return res.status(400).json({
        resourceType: "OperationOutcome",
        issue: [
    ...
    matchMixin.addPatient(
      clientID,
      patientsBundle,
      (error, responseBundle, responseHeaders, operationSummary) => {
        const auditBundle = matchMixin.createAddPatientAudEvent(
          operationSummary,
          req
        );
          ...
          () => {
            ...
            for (let entry of responseBundle.entry) {
              ...
              if (exists === -1) {
                ...
                let replaceIndex = filteredResponseBundle.findIndex((fil) => {
                  return (
                    parseInt(fil.response.etag) <
                      parseInt(entry.response.etag) &&
                    fil.response.location.startsWith(
                      entry.response.location.split("/_history")[0]
                    )
                  );
