          ...
          .then(() => {
            if (existingPatients.entry.length === 0 && !invalidId) {
              logger.info(
                ...
                )} doesnt exist, adding to the database`
              );
              ...
              async.series(
                [
                  ...
                  callback => {
                    ...
                    logger.info(
                      'Breaking all links of the new patient to other patients'
                    );
                    fhirWrapper.saveResource({
                      resourceData: bundle,
                    },
                    ...
                    Promise.all(promises)
                      .then(() => {
                        if (bundle.entry.length > 0) {
                          logger.info(
                            `Breaking ${bundle.entry.length} links of patients pointing to new patient`
                          );
                          fhirWrapper.saveResource({
                            resourceData: bundle,
                          },
